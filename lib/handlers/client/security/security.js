...
exports.W3CXMLEncryption = W3CXMLEncryption
...
SecurityClientHandler.prototype.encryptionHandler = function(handler) {
...
SecurityClientHandler.prototype.send = function(ctx, callback) {
  ...
  //My project I already have a wsse:security header and so I don't want to append a new one.
  //I make some Assertions there!
  ...
  // Only try applying Encryption if a Handler is configured
  if(this.encryptionHandler && this.encryptionHandler.applyMe) {
      var doc = this.encryptionHandler.applyMe(doc, this, (encDoc) => {
          ctx.request = encDoc.toString()
...
X509BinarySecurityToken.prototype.extractBase64Key = function(key) {
  if (key.indexOf(BEGIN_CERT) === -1 || key.indexOf(END_CERT) === -1) throw new Error("Provided PEM-format key does not contain certificate information! Try appending the certificate to the end of the file.");
...
function W3CXMLEncryption(options) {
    this.options = options

    this.reference = [];

    this.xmlenc = require('xml-encryption');

    if (!this.options.rsa_pub)
        throw new Error("Require RSA Public Key");
    if (!this.options.pem)
        throw new Error("Require CA Certificate");


}
W3CXMLEncryption.prototype.setReferences = function(ref) {
    this.reference = ref;
}

W3CXMLEncryption.prototype.getKey = function() {
    return this.options.rsa_pub
}

W3CXMLEncryption.prototype.getId = function() {
    return this.id
}

W3CXMLEncryption.prototype.extractBase64Key = function(key) {
    console.log(key);
    if (key.indexOf(BEGIN_CERT) === -1 || key.indexOf(END_CERT) === -1) throw new Error("Provided PEM-format key does not contain certificate information! Try appending the certificate to the end of the file.");
    var start = key.indexOf(BEGIN_CERT) + BEGIN_CERT.length
    var end = key.indexOf(END_CERT)
...
W3CXMLEncryption.prototype.applyMe = function(doc, security, callback) {
    console.log("W3CXMLEncryption.applyMe");
    this.id = security.getNextId()
        ...
        (ref) => {
            ...
            promises.push(
                new Promise( (resolve, reject) => {
                    var nodes = select(doc, ref);
                    this.xmlenc.encrypt(nodes.toString(), this.options, function(err, result) {
                        if(err) {
                        ...
                        // nodes[0].parentNode.removeChild(nodes[0].parentNode.lastChild);
                        nodes[0].replaceChild(myDom.parseFromString(result), nodes[0])
                        resolve();
                    });
                })
            );
